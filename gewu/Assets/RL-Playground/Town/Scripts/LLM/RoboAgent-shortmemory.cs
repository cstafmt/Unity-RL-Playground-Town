...
public class RobotAgent : MonoBehaviour
{
    ...
    public RoboChatController chatController; // 新增：拖拽 ChatController
    ...
    private async Task PlanNextMoveAsync()
    {
        ...
        // 1. 构建 Prompt
        string recentMemories = string.Join("\n", memoryStream.TakeLast(maxShortTermMemories));
        string mapInfo = townMap != null ? townMap.GetLocationListPrompt() : "Park, Bakery, Home";

        var msgs = new List<LLMClient.Message>
        {
            new LLMClient.Message { role = "system", content =
                $"你扮演 {robotName}。{personaDescription}\n" +
                $"你需要根据你的【记忆流】决定下一步去哪里。\n" +
                $"规则：\n" +
                $"1. 返回格式必须是JSON，包含 'reason'(你的思考) 和 'destination'(从地图列表中选一个地点)。\n" +
                $"2. 不要总是去同一个地方，要体现随机性和探索欲。"
            },
    ...
    public async void OnUserInteract(string userText)
    {
        ...
        // 2. UI 反馈
        UpdateThoughtUI("正在思考如何回答...", "💬");

        // 3. 构建 Prompt
        string recentMemories = string.Join("\n", memoryStream.TakeLast(5));
        var msgs = new List<LLMClient.Message>
        {
            new LLMClient.Message { role = "system", content = $"{personaDescription} 请用简短、口语化的中文回答用户。基于你的记忆回答。" },
            new LLMClient.Message { role = "user", content = $"最近记忆：\n{recentMemories}\n\n用户对你说：{userText}" }
        };
        ...
        if (!string.IsNullOrEmpty(reply))
        {
            ...
            {
                chatController.AppendHistory($"<color=cyan>{robotName}:</color> {reply}");
            }
        }
    }
}
